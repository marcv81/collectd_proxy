...
TEST3_KEY = "secret".encode()
TEST3_DATA = "message".encode()
...
class CryptoTest(unittest.TestCase):
    def test_sha1_hash(self):
        ...
        self.assertEqual(TEST1_SHA1_HASH, digest)

    def test_sha256_hash(self):
        digest = crypto.sha256_hash(TEST1_DATA)
        self.assertEqual(32, len(digest))
        self.assertEqual(TEST1_SHA256_HASH, digest)

    def test_aes256_decrypt(self):
        decrypted1 = crypto.aes256_decrypt(TEST2_KEY, TEST2_IV, TEST2_ENCRYPTED1)
        self.assertEqual(TEST2_DECRYPTED1, decrypted1)
        decrypted2 = crypto.aes256_decrypt(TEST2_KEY, TEST2_IV, TEST2_ENCRYPTED2)
        self.assertEqual(TEST2_DECRYPTED2, decrypted2)

    def test_hmac_sha256_sign(self):
        signature = crypto.hmac_sha256_sign(TEST3_KEY, TEST3_DATA)
        self.assertEqual(32, len(signature))
        self.assertEqual(TEST3_SIGNATURE, signature)
